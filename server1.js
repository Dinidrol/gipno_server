...
io.on('connection', function (socket) {
    ...
    socket.on('StartGame', function(data){
        ...
        if(!GameStatus && data.GameStatus == true){
            GameStatus = true
            if(!TimerGameId){
                TimerGameId = setInterval(TimerGame,1000)
            }
...
function TimerGame()
{
    --time
    let timeL = time / 60
    let minute = parseInt(timeL)
    let second = Math.round((timeL - minute) *60)

    client.publish('/TipsWebDisplay/TimerGame', time.toString())
